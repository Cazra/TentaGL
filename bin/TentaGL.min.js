...
p=parseInt(k[3])-1,5==k.length?(n=parseInt(k[4])-1,e.addFaceQuad(g,l,p,n)):e.addFace(g,l,p);else if("usemtl"!=k[0])throw Error("Could not read wavefront object. Error at line "+(m+1));f++;g=k[0]}}c[d]=e;b(c)},isLoading:function(){return 0<this._numLoading},getNumLoading:function(){return this._numLoading}};TentaGL.Vertex=function(a,b,c){this._xyz=vec4.fromValues(a,b,c,1)};
...
hasTexST:function(){return void 0!=this._texST},setTexST:function(a,b){this._texST=vec2.fromValues(a,b)},getTexS:function(){try{return this._texST[0]}catch(a){console.log(a.stack)}},getTexT:function(){return this._texST[1]},getNormal:function(){if(void 0===this._normal)throw Error("Vertex surface normal has not been defined.");return vec3.clone(this._normal)},hasNormal:function(){return void 0!=this._normal},setNormal:function(a,b,c){this._normal=vec3.fromValues(a,b,c);this._normal=vec3.normalize(this._normal,
this._normal)},getTangental:function(){if(void 0===this._tangental)throw Error("Vertex surface tangental has not been defined.");return vec3.clone(this._tangental)},hasTangental:function(){return void 0!==this._tangental},computeTangental:function(a,b){var c=vec3.sub(vec3.create(),a.getXYZ(),this.getXYZ()),d=vec3.sub(vec3.create(),b.getXYZ(),this.getXYZ()),e=a.getTexS()-this.getTexS(),f=b.getTexS()-this.getTexS(),g=a.getTexT()-this.getTexT(),h=b.getTexT()-this.getTexT(),e=h*e-g*f;if(0==e)return vec3.fromValues(1,
0,0);f=vec3.create();vec3.scale(f,c,h);vec3.sub(f,f,vec3.scale(vec3.create(),d,g));vec3.scale(f,f,1/e);return 0==vec3.length(f)?vec3.fromValues(1,0,0):f},setTangental:function(a){this._tangental=vec3.fromValues(a[0],a[1],a[2]);this._tangental=vec3.normalize(this._tangental,this._tangental)},clone:function(){return this.transform(mat4.create())},transform:function(a){var b=vec4.transformMat4(vec4.create(),this.getXYZ(),a),b=new TentaGL.Vertex(b[0],b[1],b[2]);if(void 0!==this._normal){var c=this.getNormal(),
c=vec4.fromValues(c[0],c[1],c[2],0),c=vec4.transformMat4(c,c,a);b.setNormal(c[0],c[1],c[2])}void 0!==this._tangental&&(c=this.getTangental(),c=vec4.fromValues(c[0],c[1],c[2],0),c=vec4.transformMat4(c,c,a),b.setTangental(c[0],c[1],c[2]));void 0!==this._texST&&b.setTexST(this.getTexS(),this.getTexT());return b}};
...
c._mvpUni=c.getUniform("mvpTrans");c._mvUni=c.getUniform("mvTrans");c._vUni=c.getUniform("vTrans");c._normalUni=c.getUniform("normalTrans");c._colorUni=c.getUniform("solidColor");c._texUni=c.getUniform("tex");c._useTexUni=c.getUniform("useTex");c._materialUni={};c._materialUni.diff=c.getUniform("m.diff");c._materialUni.spec=c.getUniform("m.spec");c._materialUni.amb=c.getUniform("m.amb");c._materialUni.emis=c.getUniform("m.emis");c._materialUni.shininess=c.getUniform("m.shininess");c._lightsUni=[];
for(var f=0;f<TentaGL.PerVertexPhongShader.MAX_LIGHTS;f++){var g=c._lightsUni[f]={},h="lights["+f+"]";g.type=c.getUniform(h+".type");g.pos=c.getUniform(h+".pos");g.dir=c.getUniform(h+".dir");g.diff=c.getUniform(h+".diff");g.spec=c.getUniform(h+".spec");g.amb=c.getUniform(h+".amb");g.attenA=c.getUniform(h+".attenA");g.attenB=c.getUniform(h+".attenB");g.attenC=c.getUniform(h+".attenC");g.cutOffAngleCos=c.getUniform(h+".cutOffAngleCos");g.spotExp=c.getUniform(h+".spotExp")}c._numLightsUni=c.getUniform("numLights")})};
TentaGL.PerVertexPhongShader.MAX_LIGHTS=16;TentaGL.PerVertexPhongShader.LIGHT_AMB=1;TentaGL.PerVertexPhongShader.LIGHT_PT=2;TentaGL.PerVertexPhongShader.LIGHT_DIR=3;TentaGL.PerVertexPhongShader.LIGHT_SPOT=4;
...
TentaGL.Vertex.prototype.getTangental);c._mvpUni=c.getUniform("mvpTrans");c._mvUni=c.getUniform("mvTrans");c._vUni=c.getUniform("vTrans");c._normalUni=c.getUniform("normalTrans");c._colorUni=c.getUniform("solidColor");c._texUni=c.getUniform("tex");c._useTexUni=c.getUniform("useTex");c._materialUni={};c._materialUni.diff=c.getUniform("m.diff");c._materialUni.spec=c.getUniform("m.spec");c._materialUni.amb=c.getUniform("m.amb");c._materialUni.emis=c.getUniform("m.emis");c._materialUni.shininess=c.getUniform("m.shininess");
c._lightUni={};c._lightUni.type=c.getUniform("light.type");c._lightUni.pos=c.getUniform("light.pos");c._lightUni.dir=c.getUniform("light.dir");c._lightUni.diff=c.getUniform("light.diff");c._lightUni.spec=c.getUniform("light.spec");c._lightUni.amb=c.getUniform("light.amb");c._lightUni.attenA=c.getUniform("light.attenA");c._lightUni.attenB=c.getUniform("light.attenB");c._lightUni.attenC=c.getUniform("light.attenC");c._lightUni.cutOffAngleCos=c.getUniform("light.cutOffAngleCos");c._lightUni.spotExp=
c.getUniform("light.spotExp")})};TentaGL.PerVertexPhongShaderMac.LIGHT_AMB=1;TentaGL.PerVertexPhongShaderMac.LIGHT_PT=2;TentaGL.PerVertexPhongShaderMac.LIGHT_DIR=3;TentaGL.PerVertexPhongShaderMac.LIGHT_SPOT=4;
...
c.getUniform(h+".spotExp")}c._numLightsUni=c.getUniform("numLights")})};TentaGL.PhongShader.MAX_LIGHTS=16;TentaGL.PhongShader.LIGHT_AMB=1;TentaGL.PhongShader.LIGHT_PT=2;TentaGL.PhongShader.LIGHT_DIR=3;TentaGL.PhongShader.LIGHT_SPOT=4;
...
c._lightUni.spotExp=c.getUniform("light.spotExp")})};TentaGL.PhongShaderMac.LIGHT_AMB=1;TentaGL.PhongShaderMac.LIGHT_PT=2;TentaGL.PhongShaderMac.LIGHT_DIR=3;TentaGL.PhongShaderMac.LIGHT_SPOT=4;
...
TentaGL.SimpleShader=function(a){var b=TentaGL.ShaderLib.getDefaultShaderPath(a),c=this;TentaGL.ShaderLoader.load(b+"simple.vert",b+"simple.frag",function(b,e){console.log("\nCreating SimpleShader");TentaGL.ShaderProgram.call(c,a,b,e);c.setAttrGetter("vertexPos",TentaGL.Vertex.prototype.getXYZ);c.setAttrGetter("vertexNormal",TentaGL.Vertex.prototype.getNormal);c.setAttrGetter("vertexTexCoords",TentaGL.Vertex.prototype.getTexST);c._mvpUni=c.getUniform("mvpTrans");c._normalUni=c.getUniform("normalTrans");
c._colorUni=c.getUniform("solidColor");c._texUni=c.getUniform("tex");c._useTexUni=c.getUniform("useTex")})};TentaGL.SimpleShader.prototype={constructor:TentaGL.SimpleShader,isaSimpleShader:!0,setMVPTrans:function(a,b){this._mvpUni.set(a,b)},setNormalTrans:function(a,b){this._normalUni.set(a,b)},setColor:function(a,b){this._colorUni.set(a,b);this._useTexUni.set(a,[0])},setTex:function(a,b){this._texUni.set(a,[b]);this._useTexUni.set(a,[1])}};

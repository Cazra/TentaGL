...
TentaGL.Level.prototype={constructor:TentaGL.Level,getApp:function(){return this._app},keyboard:function(){return this._app.keyboard()},mouse:function(){return this._app.mouse()},clean:function(a){},reset:function(a){},update:function(a){},render:function(a){}};
...
TentaGL.Model.Cone=function(a,b,c){var d=new TentaGL.Model;c||(c=TentaGL.TAU/20);for(var e=Math.floor(TentaGL.TAU/c)+1,f=0;f<e;f++){var g=TentaGL.Math.toCartesian([a,f*c,0]),k=g[0],m=g[2],g=(k/a+1)/2,h=(m/a+1)/2,k=new TentaGL.Vertex(k,0,m);k.setNormal(0,-1,0);k.setTexST(g,h);d.addVertex(k)}for(f=0;f<e;f++){var g=f*c,l=TentaGL.Math.toCartesian([a,g,0]),h=l[0],m=l[2],k=(f+.5)*c,q=TentaGL.Math.toCartesian([a,k,0]),n=q[0],s=q[2],l=vec3.sub(vec3.create(),[0,b,0],l),r=vec3.fromValues(h,0,m),r=vec3.cross(vec3.create(),
r,l),l=vec3.cross(vec3.create(),l,r);vec3.normalize(l,l);q=vec3.sub(vec3.create(),[0,b,0],q);n=vec3.fromValues(n,0,s);n=vec3.cross(vec3.create(),n,q);n=vec3.cross(vec3.create(),q,n);vec3.normalize(n,n);h=new TentaGL.Vertex(h,0,m);h.setTexST(g/TentaGL.TAU,0);h.setNormal(l[0],l[1],l[2]);g=new TentaGL.Vertex(0,b,0);g.setTexST(k/TentaGL.TAU,1);g.setNormal(n[0],n[1],n[2]);d.addVertex(h);d.addVertex(g)}l=TentaGL.Math.toCartesian([a,0,0]);h=l[0];m=l[2];l=vec3.sub(vec3.create(),[0,b,0],l);r=vec3.fromValues(h,
0,m);r=vec3.cross(vec3.create(),r,l);l=vec3.cross(vec3.create(),l,r);vec3.normalize(l,l);h=new TentaGL.Vertex(h,0,m);h.setTexST(TentaGL.TAU,0);h.setNormal(l[0],l[1],l[2]);d.addVertex(h);for(a=2;a<e;a++)b=0,c=a-1,f=a,d.addFace(b,f,c);for(a=0;a<2*e;a+=2)b=e+(a+0),c=e+(a+2),f=e+(a+1),d.addFace(b,c,f);return d};
...
TentaGL.Model.Sphere=function(a,b,c){var d=new TentaGL.Model;b||(b=TentaGL.TAU/20);c||(c=TentaGL.TAU/20);for(var e=Math.floor(TentaGL.TAU/2/b)+1,f=Math.floor(TentaGL.TAU/c)+1,g=0;g<e;g++)for(var k=g*b-TentaGL.TAU/4,m=0;m<f;m++){var h=m*c,l=TentaGL.Math.toCartesian([a,h,k]),q=l[0],n=l[1],l=l[2],h=h/TentaGL.TAU,s=k/(TentaGL.TAU/2)+.5,r=new TentaGL.Vertex(q,n,l);r.setNormal(q,n,l);r.setTexST(h,s);d.addVertex(r)}for(g=0;g<e-1;g++)for(m=0;m<f-1;m++)d.addFaceQuad((g+0)*f+(m+0),(g+0)*f+(m+1),(g+1)*f+(m+
...
h.replace("  "," ");if(""!=h&&"#"!=h.charAt(0)){h=h.split(" ");h[0]!=g&&(f=0);if("g"==h[0])c[d]=e,e=new TentaGL.Model,d=h[1];else if("v"==h[0]){var l=parseFloat(h[1]),q=parseFloat(h[2]),n=parseFloat(h[3]);e.addVertex(new TentaGL.Vertex(l,q,n))}else if("vn"==h[0])g=e.getVertex(f),l=parseFloat(h[1]),q=parseFloat(h[2]),n=parseFloat(h[3]),g.setNormal(l,q,n);else if("vt"==h[0])g=e.getVertex(f),l=parseFloat(h[1]),q=parseFloat(h[2]),g.setTexST(l,q);else if("f"==h[0])g=parseInt(h[1])-1,l=parseInt(h[2])-1,
q=parseInt(h[3])-1,5==h.length?(n=parseInt(h[4])-1,e.addFaceQuad(g,l,q,n)):e.addFace(g,l,q);else if("usemtl"!=h[0])throw Error("Could not read wavefront object. Error at line "+(m+1));f++;g=h[0]}}c[d]=e;b(c)},isLoading:function(){return 0<this._numLoading},getNumLoading:function(){return this._numLoading}};TentaGL.Vertex=function(a,b,c){this._xyz=vec4.fromValues(a,b,c,1)};
...
TentaGL.RGBAFilter.OutlineColor.prototype={constructor:TentaGL.RGBAFilter.OutlineColor,getColor:function(){return this._color},filter:function(a,b,c,d,e){var f=b[c],g=b[c+1],k=b[c+2],m=b[c+3];if(255!=m)for(var h=TentaGL.PixelData.indexToXY(c,d),l=h[0],h=h[1],q=-1;1>=q;q++){var n=h+q;if(!(n>=e||0>n))for(var s=-1;1>=s;s++){var r=l+s;if(!(r>=d||0>r)&&(r=TentaGL.PixelData.xyToIndex(r,n,d),255==b[r+3]))return f=this._color.getRedByte(),g=this._color.getGreenByte(),k=this._color.getBlueByte(),this.setPixel(a,
...
TentaGL.BlitteredFont.fromFont=function(a,b,c,d,e){var f="monospace"==a.getType(),g=a.getStringDimensions("W"),k=g[0],g=g[1];c=new TentaGL.BlitteredFont(void 0,f,k,g,c,d);d=1.5<b.getRed()+b.getGreen()+b.getBlue()?new TentaGL.Color.RGBA(0,0,0,1):new TentaGL.Color.RGBA(1,1,1,1);for(var m=TentaGL.Canvas2D.create(16*(k+2),6*(g+2),d),h=32;128>h;h++){var l=1+(h-32)%16*(k+2),q=1+Math.floor((h-32)/16)*(g+2),n=String.fromCharCode(h),s=TentaGL.Canvas2D.create(k,g,d);TentaGL.Canvas2D.drawString(s,n,a,b,0,0);
TentaGL.Canvas2D.drawImage(m,s,l,q)}a=TentaGL.PixelData.fromCanvas(m);a=a.filter(new TentaGL.RGBAFilter.OneColor(b,.5));e&&(a=e(a));c._createChars(a,f,k,g);return c};TentaGL.Font=function(a,b,c){this._name=a;this._type=b;this._size=c;this._italic=this._bold=!1;this._vPad=1};

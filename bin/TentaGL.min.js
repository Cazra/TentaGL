...
a+" out of bounds.");if(0>b||b>=this._vertices.length)throw Error("Index "+b+" out of bounds.");this._indices.push(a);this._indices.push(b);var c=this._vertices[a],d=this._vertices[b];c.tangental(c.computeTangental(d,d));d.tangental(d.computeTangental(c,c))},addLineStrip:function(a){if(2>a.length)throw Error("Not enough indices in line strip array.");for(var b=a[0],c=1;c<a.length;c++)this.addLine(b,a[c])},addLineLoop:function(a){if(2>a.length)throw Error("Not enough indices in line strip array.");
...
b.getCullMode());a.bindBuffer(GL_ELEMENT_ARRAY_BUFFER,e);a.drawElements(f,b.numIndices(),GL_UNSIGNED_SHORT,0)}};TentaGL.Vertex=function(a){this._xyz=vec3.copy([],a);this._xyz[3]=1;this._texST=[0,0];this._normal=[0,0,0];this._tangental=[0,0,0]};

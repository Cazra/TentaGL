...
getWidth:function(a){return a._scissorWidth},getHeight:function(a){return a._scissorHeight},isEnabled:function(a){return a._scissorEnabled}};
...
d);a.renderbufferStorage(a.RENDERBUFFER,a.STENCIL_INDEX8,b,c);a.bindRenderbuffer(a.RENDERBUFFER,null);return d}};TentaGL.Viewport={get:function(a){return[a._viewX,a._viewY,a._viewWidth,a._viewHeight]},getX:function(a){return a._viewX},getY:function(a){return a._viewY},getWidth:function(a){return a._viewWidth},getHeight:function(a){return a._viewHeight},set:function(a,b){a._viewX=b[0];a._viewY=b[1];a._viewWidth=b[2];a._viewHeight=b[3];a.viewport(b[0],b[1],b[2],b[3])}};TentaGL.TAU=2*Math.PI;
...
TentaGL.Math.InfinitePlane=function(a,b){b||(b=[0,0,0]);this._normal=vec3.clone(a);this._nHat=vec3.normalize(vec3.create(),this._normal);this._pt=vec3.clone(b)};
TentaGL.Math.InfinitePlane.prototype={constructor:TentaGL.Math.InfinitePlane,isaInfinitePlane:!0,getNormal:function(){return this._normal},getUnitNormal:function(){return this._nHat},getPoint:function(){return this._pt},distToPt:function(a){var b=vec3.sub(vec3.create(),a,this._pt);a=vec3.len(b);if(0==a)return 0;b=vec3.normalize(vec3.create(),b);b=vec3.dot(b,this._nHat);return Math.abs(a*b)},getCoefficients:function(){var a=this._normal[0],b=this._normal[1],c=this._normal[2];return[a,b,c,-a*this._pt[0]-
b*this._pt[1]-c*this._pt[2]]},getParallelVector:function(){var a=this._pt,b;if(0==this._normal[0])b=[a[0]+1,p[1],p[2]];else if(0==this._normal[1])b=[a[0],p[1]+1,p[2]];else if(0==this._normal[2])b=[a[0],p[1],p[2]+1];else{b=this.getCoefficients();var c=a[0]+1;b=[c,0,(-b[0]*c-b[3])/b[2]]}return vec3.sub(vec3.create(),b,a)},containsPt:function(a,b){b||(b=0);var c=this.getCoefficients();return Math.abs(c[0]*a[0]+c[1]*a[1]+c[2]*a[2]+c[3])<b},ptRelative:function(a){a=vec3.sub(vec3.create(),a,this._pt);vec3.normalize(a,
a);a=vec3.dot(this._nHat,a);return 0<a?1:0>a?-1:0},ptIsAbove:function(a){return 1==ptRelative(a)},ptIsBelow:function(a){return-1==ptRelative(a)}};TentaGL.Math.Line2D=function(a,b){this._p1=vec3.clone(a);this._p1[2]=1;this._p2=vec3.clone(b);this._p2[2]=1;this._length=vec2.dist(a,b)};
...
a,this._p2);if(0>=vec3.dot(b,c))return vec3.len(c);if(0<=vec3.dot(b,a))return vec3.len(a);b=vec3.normalize(vec3.create(),b);a=vec3.normalize(vec3.create(),c);b=vec3.cross(vec3.create(),b,a);return vec3.len(c)*vec3.len(b)}};TentaGL.Math.Sphere=function(a,b){b||(b=[0,0,0]);this._pt=vec3.clone(b);this._radius=a};
...
TentaGL.Level.prototype={constructor:TentaGL.Level,getApp:function(){return this._app},keyboard:function(){return this._app.keyboard()},mouse:function(){return this._app.mouse()},clean:function(a){},reset:function(a){},update:function(a){},render:function(a){}};TentaGL.Camera=function(){};
...
(b/c),-1*(a[1]-c/2),0);vec3.scale(a,a,2/c);a[2]=0-this._dist;b=this.getProjectionTransform(b/c);vec3.transformMat4(a,a,mat4.invert(mat4.create(),b));b=this.getUp();c=this.getRight();return vec3.fromValues((a[0]*c[0]+a[1]*b[0])*this._dist+this._center[0],(a[0]*c[1]+a[1]*b[1])*this._dist+this._center[1],(a[0]*c[2]+a[1]*b[2])*this._dist+this._center[2])}};Util.Inheritance.inherit(TentaGL.Camera3D,TentaGL.Camera);
...
TentaGL.Model.Cylinder=function(a,b,c){var d=new TentaGL.Model(!1);c||(c=TentaGL.TAU/20);for(var e=Math.floor(TentaGL.TAU/c)+1,f=0;f<e;f++){var g=f*c,k=TentaGL.Math.toCartesian([a,g,0]),l=k[0],k=k[2],g=(l/a+1)/2,h=(k/a+1)/2,m=new TentaGL.Vertex(l,0,k);m.setNormal(0,-1,0);m.setTexST(g,h);d.addVertex(m)}for(f=0;f<e;f++)g=f*c,k=TentaGL.Math.toCartesian([a,g,0]),l=k[0],k=k[2],g/=TentaGL.TAU,m=new TentaGL.Vertex(l,0,k),m.setNormal(l,0,k),m.setTexST(g,0),d.addVertex(m);for(f=0;f<e;f++)g=f*c,k=TentaGL.Math.toCartesian([a,
g,0]),l=k[0],k=k[2],g/=TentaGL.TAU,m=new TentaGL.Vertex(l,b,k),m.setNormal(l,0,k),m.setTexST(g,1),d.addVertex(m);for(f=0;f<e;f++)g=f*c,k=TentaGL.Math.toCartesian([a,g,0]),l=k[0],k=k[2],g=(l/a+1)/2,h=(k/a+1)/2,m=new TentaGL.Vertex(l,b,k),m.setNormal(0,1,0),m.setTexST(g,h),d.addVertex(m);for(a=2;a<e;a++)b=0,c=a-1,f=a,d.addFace(b,f,c);for(a=0;a<e-1;a++)b=1*e+(a+0),c=1*e+(a+1),f=2*e+(a+1),d.addFaceQuad(b,c,f,2*e+(a+0));for(a=2;a<e;a++)b=3*e,c=3*e+a-1,f=3*e+a,d.addFace(b,c,f);return d};
...
TentaGL.Model.Sphere=function(a,b,c){var d=new TentaGL.Model;b||(b=TentaGL.TAU/20);c||(c=TentaGL.TAU/20);for(var e=Math.floor(TentaGL.TAU/2/b)+1,f=Math.floor(TentaGL.TAU/c)+1,g=0;g<e;g++)for(var k=g*b-TentaGL.TAU/4,l=0;l<f;l++){var h=l*c,m=TentaGL.Math.toCartesian([a,h,k]),n=m[0],q=m[1],m=m[2],h=h/TentaGL.TAU,r=k/(TentaGL.TAU/2)+.5,s=new TentaGL.Vertex(n,q,m);s.setNormal(n,q,m);s.setTexST(h,r);d.addVertex(s)}for(g=0;g<e-1;g++)for(l=0;l<f-1;l++)d.addFaceQuad((g+0)*f+(l+0),(g+0)*f+(l+1),(g+1)*f+(l+
...
TentaGL.Model.ObjReader={_numLoading:0,fromURL:function(a,b){var c=this,d=new XMLHttpRequest;d.open("get",a);d.onerror=function(a){console.log("AJAX error: ",a);c._numLoading--};d.onreadystatechange=function(a){4==this.readyState&&200==this.status&&(c._processResponse(d.responseText,b),c._numLoading--)};d.send();this._numLoading++},_processResponse:function(a,b){for(var c=[],d="",e=new TentaGL.Model,f=0,g="",k=a.split("\n"),l=0;l<k.length;l++){for(var h=k[l].trim(),h=h.replace("\t"," ");-1!=h.indexOf("  ");)h=
h.replace("  "," ");if(""!=h&&"#"!=h.charAt(0)){h=h.split(" ");h[0]!=g&&(f=0);if("g"==h[0])c[d]=e,e=new TentaGL.Model,d=h[1];else if("v"==h[0]){var m=parseFloat(h[1]),n=parseFloat(h[2]),q=parseFloat(h[3]);e.addVertex(new TentaGL.Vertex(m,n,q))}else if("vn"==h[0])g=e.getVertex(f),m=parseFloat(h[1]),n=parseFloat(h[2]),q=parseFloat(h[3]),g.setNormal(m,n,q);else if("vt"==h[0])g=e.getVertex(f),m=parseFloat(h[1]),n=parseFloat(h[2]),g.setTexST(m,n);else if("f"==h[0])g=parseInt(h[1])-1,m=parseInt(h[2])-1,
n=parseInt(h[3])-1,5==h.length?(q=parseInt(h[4])-1,e.addFaceQuad(g,m,n,q)):e.addFace(g,m,n);else if("usemtl"!=h[0])throw Error("Could not read wavefront object. Error at line "+(l+1));f++;g=h[0]}}c[d]=e;b(c)},isLoading:function(){return 0<this._numLoading},getNumLoading:function(){return this._numLoading}};TentaGL.Vertex=function(a,b,c){this._xyz=vec4.fromValues(a,b,c,1)};
...
this._tangental)throw Error("Vertex surface tangental has not been defined.");return vec3.clone(this._tangental)},computeTangental:function(a,b){var c=vec3.fromValues(a.getX()-this.getX(),a.getY()-this.getY(),a.getZ()-this.getZ()),d=vec3.fromValues(b.getX()-this.getX(),b.getY()-this.getY(),b.getZ()-this.getZ()),e=a.getTexS()-this.getTexS(),f=b.getTexS()-this.getTexS(),g=a.getTexT()-this.getTexT(),k=b.getTexT()-this.getTexT(),e=k*e-g*f;if(0==e)return vec3.fromValues(1,0,0);f=vec3.create();vec3.scale(f,
c,k);vec3.sub(f,f,vec3.scale(vec3.create(),d,g));vec3.scale(f,f,1/e);return f},setTangental:function(a){this._tangental=vec3.fromValues(a[0],a[1],a[2]);this._tangental=vec3.normalize(this._tangental,this._tangental)},clone:function(){return this.transform(mat4.create())},transform:function(a){var b=vec4.transformMat4(vec4.create(),this.getXYZ(),a),b=new TentaGL.Vertex(b[0],b[1],b[2]);if(void 0!==this._normal){var c=this.getNormal(),c=vec4.fromValues(c[0],c[1],c[2],0),c=vec4.transformMat4(c,c,a);b.setNormal(c[0],
c[1],c[2])}void 0!==this._tangental&&(c=this.getTangental(),c=vec4.fromValues(c[0],c[1],c[2],0),c=vec4.transformMat4(c,c,a),b.setTangental(c[0],c[1],c[2]));void 0!==this._texST&&b.setTexST(this.getTexS(),this.getTexT());return b}};TentaGL.ImageLoader={_numLoading:0,load:function(a,b,c){var d=this;this._numLoading++;c||(c=function(){d._numLoading--;throw Error("Could not load image at "+a+".");});var e=new Image;e.onload=function(){b(e);d._numLoading--};e.src=a},isLoading:function(){return 0<this._numLoading}};
TentaGL.PixelData=function(a,b,c){this._pixels=a;this._width=b;this._height=c};
...
TentaGL.RGBAFilter.Convolution.prototype={constructor:TentaGL.RGBAFilter.Convolution,getMatrix:function(){return this._matrix},getSideLength:function(){return this._sideLen},filter:function(a,b,c,d,e){for(var f=0,g=0,k=0,l=b[c+3],h=this._sideLen,m=TentaGL.PixelData.indexToXY(c,d),n=m[0],m=m[1],q=Math.floor(h/2),r=0;r<h;r++){var s=m+r-q;s>=e&&(s=e-1);0>s&&(s=0);for(var u=0;u<h;u++){var w=this._matrix[r][u],t=n+u-q;t>=d&&(t=d-1);0>t&&(t=0);var t=TentaGL.PixelData.xyToIndex(t,s,d),x=b[t+3]/255,f=f+b[t]*
w*x,g=g+b[t+1]*w*x,k=k+b[t+2]*w*x}}f=TentaGL.Math.clamp(f,-256,255);0>f&&(f+=256);g=TentaGL.Math.clamp(g,-256,255);0>g&&(g+=256);k=TentaGL.Math.clamp(k,-256,255);0>k&&(k+=256);this.setPixel(a,c,f,g,k,l)}};Util.Inheritance.inherit(TentaGL.RGBAFilter.Convolution,TentaGL.RGBAFilter);TentaGL.RGBAFilter.Convolution.Blur3=function(){return new TentaGL.RGBAFilter.Convolution([[1,1,1],[1,1,1],[1,1,1]])};
...
TentaGL.RGBAFilter.OutlineColor.prototype={constructor:TentaGL.RGBAFilter.OutlineColor,getColor:function(){return this._color},filter:function(a,b,c,d,e){var f=b[c],g=b[c+1],k=b[c+2],l=b[c+3];if(255!=l)for(var h=TentaGL.PixelData.indexToXY(c,d),m=h[0],h=h[1],n=-1;1>=n;n++){var q=h+n;if(!(q>=e||0>q))for(var r=-1;1>=r;r++){var s=m+r;if(!(s>=d||0>s)&&(s=TentaGL.PixelData.xyToIndex(s,q,d),255==b[s+3]))return f=this._color.getRedByte(),g=this._color.getGreenByte(),k=this._color.getBlueByte(),this.setPixel(a,
c,f,g,k,255)}}this.setPixel(a,c,f,g,k,l)}};TentaGL.RGBAFilter.OutlineColor.RGBBytes=function(a,b,c){a=TentaGL.Color.RGBABytes(a,b,c);return new TentaGL.RGBAFilter.OutlineColor(a)};Util.Inheritance.inherit(TentaGL.RGBAFilter.OutlineColor,TentaGL.RGBAFilter);TentaGL.RGBAFilter.SubColor=function(a){this._color=a};
...
TentaGL.Keyboard=function(a){this._isPressed=[];this._justPressed=[];this._justReleased=[];this._repPressed=[];this._pressedSinceLast=[];this._releasedSinceLast=[];var b=this;a.addEventListener("keydown",function(a){b._pressedSinceLast[a.keyCode]=!0},!1);a.addEventListener("keyup",function(a){b._releasedSinceLast[a.keyCode]=!0},!1)};
...
var KeyCode={BACK_SPACE:8,TAB:9,ENTER:13,SHIFT:16,CTRL:17,ALT:18,PAUSE:19,CAPS_LOCK:20,ESCAPE:27,PAGE_UP:33,PAGE_DOWN:34,END:35,HOME:36,LEFT:37,UP:38,RIGHT:39,DOWN:40,INSERT:45,DELETE:46,NUM0:48,NUM1:49,NUM2:50,NUM3:51,NUM4:52,NUM5:53,NUM6:54,NUM7:55,NUM8:56,NUM9:57,A:65,B:66,C:67,D:68,E:69,F:70,G:71,H:72,I:73,J:74,K:75,L:76,M:77,N:78,O:79,P:80,Q:81,R:82,S:83,T:84,U:85,V:86,W:87,X:88,Y:89,Z:90,NUMPAD0:96,NUMPAD1:97,NUMPAD2:98,NUMPAD3:99,NUMPAD4:100,NUMPAD5:101,NUMPAD6:102,NUMPAD7:103,NUMPAD8:104,
...
TentaGL.Mouse=function(a){this._xy=[0,0];this._xyPage=[0,0];this._mouseMoved=this._mouseMovedSinceLast=!1;this._pressedSinceLast=[];this._releasedSinceLast=[];this._isPressed=[];this._justPressed=[];this._justReleased=[];this._clickCount=[];this._clickCount[TentaGL.Mouse.LEFT]=0;this._clickCount[TentaGL.Mouse.MIDDLE]=0;this._clickCount[TentaGL.Mouse.MIDDLE]=0;this._lastClickTime=[];this._lastClickTime[TentaGL.Mouse.LEFT]=0;this._lastClickTime[TentaGL.Mouse.MIDDLE]=0;this._lastClickTime[TentaGL.Mouse.MIDDLE]=
0;this._multiClickTime=400;this._wheelTicksY=this._wheelTicksX=this._wheelTicksYSinceLast=this._wheelTicksXSinceLast=0;var b=this;a.onmousemove=function(c){var d=TentaGL.DOM.getAbsolutePosition(this),e=a.getBoundingClientRect(),d=TentaGL.Math.clamp(Math.floor(e.left),0,a.width),e=TentaGL.Math.clamp(Math.floor(e.top),0,a.height),d=[c.clientX-d,c.clientY-e];c=[c.pageX,c.pageY];b._xy=d;b._xyPage=c;b._mouseMovedSinceLast=!0};a.oncontextmenu=function(a){return!1};a.onmousedown=function(a){b._pressedSinceLast[a.which]=
!0;var d=Date.now(),e=d-b._lastClickTime[a.which];b._lastClickTime[a.which]=d;e<b._multiClickTime?b._clickCount[a.which]++:b._clickCount[a.which]=1};a.onmouseup=function(a){b._releasedSinceLast[a.which]=!0};a.onmousewheel=function(a){0>a.wheelDeltaY?b._wheelTicksYSinceLast--:0<a.wheelDeltaY&&b._wheelTicksYSinceLast++;0>a.wheelDeltaX?b._wheelTicksXSinceLast--:0<a.wheelDeltaX&&b._wheelTicksXSinceLast++;a.preventDefault()}};TentaGL.Mouse.LEFT=1;TentaGL.Mouse.MIDDLE=2;TentaGL.Mouse.RIGHT=3;
TentaGL.Mouse.prototype={constructor:TentaGL.Mouse,poll:function(){this._justPressed=[];this._justReleased=[];this._wheelTicksX=this._wheelTicksXSinceLast;this._wheelTicksY=this._wheelTicksYSinceLast;this._wheelTicksYSinceLast=this._wheelTicksXSinceLast=0;this._mouseMoved=this._mouseMovedSinceLast;this._mouseMovedSinceLast=!1;for(var a in this._pressedSinceLast)this._isPressed[a]||(this._justPressed[a]=!0),this._isPressed[a]=!0;for(a in this._releasedSinceLast)this._isPressed[a]=!1,this._justReleased[a]=
!0;this._pressedSinceLast=[];this._releasedSinceLast=[]},getXY:function(){return[this._xy[0],this._xy[1]]},getX:function(){return this._xy[0]},getY:function(){return this._xy[1]},getPageXY:function(){return[this._xyPage[0],this._xyPage[1]]},getPageX:function(){return this._xyPage[0]},getPageY:function(){return this._xyPage[1]},hasMoved:function(){return this._mouseMoved},isLeftPressed:function(){return this._isPressed[TentaGL.Mouse.LEFT]||!1},justLeftPressed:function(){return this._justPressed[TentaGL.Mouse.LEFT]||
!1},justLeftReleased:function(){return this._justReleased[TentaGL.Mouse.LEFT]||!1},leftClickCount:function(){return this._clickCount[TentaGL.Mouse.LEFT]},isMiddlePressed:function(){return this._isPressed[TentaGL.Mouse.MIDDLE]||!1},justMiddlePressed:function(){return this._justPressed[TentaGL.Mouse.MIDDLE]||!1},justMiddleReleased:function(){return this._justReleased[TentaGL.Mouse.MIDDLE]||!1},middleClickCount:function(){return this._clickCount[TentaGL.Mouse.MIDDLE]},isRightPressed:function(){return this._isPressed[TentaGL.Mouse.RIGHT]||
!1},justRightPressed:function(){return this._justPressed[TentaGL.Mouse.RIGHT]||!1},justRightReleased:function(){return this._justReleased[TentaGL.Mouse.RIGHT]||!1},rightClickCount:function(){return this._clickCount[TentaGL.Mouse.RIGHT]},scrollUpAmount:function(){return Math.abs(Math.max(this._wheelTicksY,0))},scrollDownAmount:function(){return Math.abs(Math.min(this._wheelTicksY,0))},scrollLeftAmount:function(){return Math.abs(Math.min(this._wheelTicksX,0))},scrollRightAmount:function(){return Math.abs(Math.max(this._wheelTicksX,
0))}};TentaGL.Light=function(a,b,c){a||(a=TentaGL.Color.RGBA(1,1,1,1));b||(b=TentaGL.Color.RGBA(1,1,1,1));c||(c=TentaGL.Color.RGBA(0,0,0,1));this._diffuse=a;this._specular=b;this._ambient=c};TentaGL.Light.prototype={constructor:TentaGL.Light,isaLight:!0,getDiffuse:function(){return this._diffuse},getSpecular:function(){return this._specular},getAmbient:function(){return this._ambient}};
TentaGL.AmbientLight=function(a){TentaGL.Light.call(this,TentaGL.Color.RGBA(0,0,0,1),TentaGL.Color.RGBA(0,0,0,1),a)};TentaGL.AmbientLight.prototype={constructor:TentaGL.AmbientLight,isaAmbientLight:!0};Util.Inheritance.inherit(TentaGL.AmbientLight,TentaGL.Light);TentaGL.DirectionalLight=function(a,b,c,d){TentaGL.Light.call(this,b,c,d);this._direction=a};
TentaGL.DirectionalLight.prototype={constructor:TentaGL.DirectionalLight,isaDirectionalLight:!0,getDirection:function(){return this._direction},setDirection:function(a){this._direction[0]=a[0];this._direction[1]=a[1];this._direction[2]=a[2]}};TentaGL.PointLight=function(a,b,c,d){TentaGL.Light.call(this,b,c,d);this._xyz=vec4.clone(a);this._xyz[3]=1};
TentaGL.PointLight.prototype={constructor:TentaGL.PointLight,isaPointLight:!0,getXYZ:function(){return this._xyz},setXYZ:function(a){this._xyz[0]=a[0];this._xyz[1]=a[1];this._xyz[2]=a[2]}};Util.Inheritance.inherit(TentaGL.PointLight,TentaGL.Light);TentaGL.Material=function(){};TentaGL.Material.prototype={isaMaterial:!0,clean:function(a){},useMe:function(a){},isLoaded:function(){}};
...
getPixelData:function(a,b,c,d,e,f){b=b||0;c=c||0;d=d||this._width-b;e=e||this._height-c;return TentaGL.PixelData.fromGLTexture(a,this._tex,b,c,d,e,f)},useMe:function(a){a.activeTexture(GL_TEXTURE0);a.bindTexture(GL_TEXTURE_2D,this._tex);TentaGL.ShaderLib.current(a).setTex(a,0)}};Util.Inheritance.inherit(TentaGL.BufferTexture,TentaGL.Material);TentaGL.Color=function(){this._rgba=vec4.create();this.setRGBA(0,0,0,1)};
...
TentaGL.Gradient=function(a,b){this._pt=vec2.clone(a);this._vec=vec2.clone(b);this._breakPts=[]};
...
e,f)},useMe:function(a){a.bindTexture(a.TEXTURE_2D,this._tex);a.activeTexture(a.TEXTURE0);TentaGL.ShaderLib.current(a).setTex(a,0)}};Util.Inheritance.inherit(TentaGL.Texture,TentaGL.Material);
...
TentaGL.VBOData=function(a,b,c){var d=[];this._byteOffsets={};for(var e=b.getVertices(),f=0;f<e.length;f++){var g=e[f],k=0,l;for(l in c){var h=c[l],m=h.getValues(g);this._byteOffsets[l]=k;k+=h.sizeBytes();for(h=0;h<m.length;h++)d.push(m[h])}}this._attrBuffer=a.createBuffer();a.bindBuffer(GL_ARRAY_BUFFER,this._attrBuffer);a.bufferData(GL_ARRAY_BUFFER,new Float32Array(d),GL_STATIC_DRAW);d=b.getIndices();this._numIndices=b.numIndices();this._elemBuffer=a.createBuffer();a.bindBuffer(GL_ELEMENT_ARRAY_BUFFER,
...
TentaGL.SceneNode=function(a){void 0===a&&(a=[0,0,0]);this._xyz=vec4.fromValues(a[0],a[1],a[2],1);this._scaleXYZ=[1,1,1];this._scaleUni=1;this._quat=quat.create();this._objLook=vec3.fromValues(0,0,1);this._objUp=vec3.fromValues(0,1,0);this._transform=mat4.create();this._isVisible=this._tranDirty=!0;this._parent=void 0};
...
b=b.getCompositeParent();return a},isVisible:function(){return this._isVisible},setVisible:function(a){this._isVisible=a},getXYZ:function(){return this._xyz},getWorldXYZ:function(){var a=this.getWorldTransform();return vec4.fromValues(a[12],a[13],a[14],1)},setXYZ:function(a){this._xyz[0]=a[0];this._xyz[1]=a[1];this._xyz[2]=a[2];this._tranDirty=!0},getX:function(){return this._xyz[0]},getY:function(){return this._xyz[1]},getZ:function(){return this._xyz[2]},setEuclidXYZ:function(a){var b=quat.create();
quat.rotateX(b,b,a[0]);quat.rotateY(b,b,a[1]);quat.rotateZ(b,b,a[2]);this.setQuat(b)},setEuclicXZY:function(a){var b=quat.create();quat.rotateX(b,b,a[0]);quat.rotateZ(b,b,a[2]);quat.rotateY(b,b,a[1]);this.setQuat(b)},setEuclicYXZ:function(a){var b=quat.create();quat.rotateY(b,b,a[1]);quat.rotateX(b,b,a[0]);quat.rotateZ(b,b,a[2]);this.setQuat(b)},setEuclidYZX:function(a){var b=quat.create();quat.rotateY(b,b,a[1]);quat.rotateZ(b,b,a[2]);quat.rotateX(b,b,a[0]);this.setQuat(b)},setEuclidZXY:function(a){var b=
quat.create();quat.rotateZ(b,b,a[2]);quat.rotateX(b,b,a[0]);quat.rotateY(b,b,a[1]);this.setQuat(b)},setEuclidZYX:function(a){var b=quat.create();quat.rotateZ(b,b,a[2]);quat.rotateY(b,b,a[1]);quat.rotateX(b,b,a[0]);this.setQuat(b)},getQuat:function(){return this._quat},getWorldQuat:function(){return this._getWorldQuat(quat.create())},_getWorldQuat:function(a){quat.mul(a,this.getQuat(),a);return this._parent?this._parent._getWorldQuat(a):a},setQuat:function(a){this._quat=a;this._tranDirty=!0},getParentWorldQuat:function(){return this._parent?
this._parent.getWorldQuat():quat.create()},rotate:function(a,b){var c=quat.setAxisAngle(quat.create(),a,b);this.setQuat(quat.mul(this._quat,c,this._quat))},mulQuat:function(a){this.setQuat(quat.mul(this._quat,a,this._quat))},getBaseLook:function(){return this._objLook},getWorldBaseLook:function(){var a=this.getParentWorldQuat();return vec3.transformQuat(vec3.create(),this._objLook,a)},setBaseLook:function(a){vec3.normalize(this._objLook,a);this._correctUpVector()},getLook:function(){return vec3.transformQuat(vec3.create(),
this._objLook,this._quat)},getWorldLook:function(){var a=this.getWorldQuat();return vec3.transformQuat(vec3.create(),this.getBaseLook(),a)},getBaseRight:function(){return vec3.cross(vec3.create(),this._objLook,this._objUp)},getWorldBaseRight:function(){var a=this.getParentWorldQuat();return vec3.transformQuat(vec3.create(),this.getBaseRight(),a)},getRight:function(){return vec3.transformQuat(vec3.create(),this.getBaseRight(),this._quat)},getWorldRight:function(){var a=this.getWorldQuat();return vec3.transformQuat(vec3.create(),
this.getBaseRight(),a)},getUp:function(){return vec3.transformQuat(vec3.create(),this._objUp,this._quat)},getWorldUp:function(){var a=this.getWorldQuat();return vec3.transformQuat(vec3.create(),this.getBaseUp(),a)},getBaseUp:function(){return this._objUp},getWorldBaseUp:function(){var a=this.getParentWorldQuat();return vec3.transformQuat(vec3.create(),this._objUp,a)},setBaseUp:function(a){vec3.normalize(this._objUp,a);this._correctUpVector()},_correctUpVector:function(){var a=this._objLook,b=this._objUp,
c=vec3.cross(vec3.create(),a,b);this._objUp=vec3.cross(b,c,a)},getBaseLookVectors:function(){return[this.getBaseLook(),this.getBaseUp(),this.getBaseRight()]},getLookVectors:function(){this.getQuat();var a=vec3.transformQuat(vec3.create(),this._objLook,this._quat),b=vec3.transformQuat(vec3.create(),this._objUp,this._quat),c=vec3.transformQuat(vec3.create(),this.getBaseRight,this._quat);return[a,b,c]},getWorldLookVectors:function(){this.getWorldQuat();var a=vec3.transformQuat(vec3.create(),this._objLook,
this._quat),b=vec3.transformQuat(vec3.create(),this._objUp,this._quat),c=vec3.transformQuat(vec3.create(),this.getBaseRight,this._quat);return[a,b,c]},orient:function(a,b){this.setQuat(TentaGL.Math.getOrientation(this.getBaseLook(),this.getBaseUp(),a,b))},orientWorld:function(a,b){var c=this.getParentWorldQuat();quat.invert(c,c);a=vec3.transformQuat(vec3.create(),a,c);b=vec3.transformQuat(vec3.create(),b,c);this.setQuat(TentaGL.Math.getOrientation(this.getBaseLook(),this.getBaseUp(),a,b))},lookAt:function(a){var b=
this.getXYZ();a=vec3.fromValues(a[0]-b[0],a[1]-b[1],a[2]-b[2]);b=vec3.cross(vec3.create(),a,this.getBaseUp());b=vec3.cross(vec3.create(),b,a);this.orient(a,b)},lookAtWorld:function(a){var b=this.getWorldXYZ();a=vec3.fromValues(a[0]-b[0],a[1]-b[1],a[2]-b[2]);b=vec3.cross(vec3.create(),a,this.getWorldBaseUp());b=vec3.cross(vec3.create(),b,a);this.orientWorld(a,b)},billboardPoint:function(a,b){var c=this.getXYZ(),c=vec3.fromValues(a[0]-c[0],a[1]-c[1],a[2]-c[2]),d=vec3.cross(vec3.create(),c,b);b=vec3.cross(vec3.create(),
d,c);this.orient(c,b)},billboardWorldPoint:function(a,b){var c=this.getWorldXYZ(),c=vec3.fromValues(a[0]-c[0],a[1]-c[1],a[2]-c[2]),d=vec3.cross(vec3.create(),c,b);b=vec3.cross(vec3.create(),d,c);this.orientWorld(c,b)},billboardAxis:function(a){var b=this.getXYZ(),c=this.getBaseLook(),d=this.getBaseUp();a=vec3.fromValues(a[0]-b[0],a[1]-b[1],a[2]-b[2]);var b=TentaGL.Math.getQuatFromTo(c,a),e=vec3.transformQuat(vec3.create(),d,b),f=vec3.cross(vec3.create(),a,d),f=vec3.cross(vec3.create(),f,a),e=TentaGL.Math.getQuatFromTo(e,
f);quat.mul(b,e,b);e=TentaGL.Math.getQuatFromTo(f,d);quat.mul(b,e,b);a=vec3.transformQuat(a,c,b);this.orient(a,d)},billboardWorldAxis:function(a){var b=this.getWorldXYZ(),c=this.getWorldBaseLook(),d=this.getWorldBaseUp();a=vec3.fromValues(a[0]-b[0],a[1]-b[1],a[2]-b[2]);var b=TentaGL.Math.getQuatFromTo(c,a),e=vec3.transformQuat(vec3.create(),d,b),f=vec3.cross(vec3.create(),a,d),f=vec3.cross(vec3.create(),f,a),e=TentaGL.Math.getQuatFromTo(e,f);quat.mul(b,e,b);e=TentaGL.Math.getQuatFromTo(f,d);quat.mul(b,
e,b);a=vec3.transformQuat(a,c,b);this.orientWorld(a,d)},billboardPlane:function(a,b){this.orient(a,b)},billboardWorldPlane:function(a,b){this.orientWorld(a,b)},getScaleXYZ:function(){return this._ScaleXYZ},setScaleXYZ:function(a){this._scaleXYZ=vec3.fromValues(a[0],a[1],a[2]);this._tranDirty=!0},getScaleX:function(){return this._scaleXYZ[0]},getScaleY:function(){return this._scaleXYZ[1]},getScaleZ:function(){return this._scaleXYZ[2]},getScaleUni:function(){return this._scaleUni},setScaleUni:function(a){this._scaleUni=
a;this._tranDirty=!0},getWorldScaleUni:function(){var a=this.getScaleUni();this._parent&&(a+=this._parent.getWorldScaleUni());return a},getModelTransform:function(){this._tranDirty&&(this._transform=this.getTRSTransform(),this._tranDirty=!1);return this._transform},getTRSTransform:function(){var a=this.getX(),b=this.getY(),c=this.getZ(),d=this.getScaleUni(),e=this.getScaleX()*d,f=this.getScaleY()*d,d=this.getScaleZ()*d,g=this._transform;mat4.fromQuat(g,this.getQuat());g[0]*=e;g[1]*=e;g[2]*=e;g[4]*=
f;g[5]*=f;g[6]*=f;g[8]*=d;g[9]*=d;g[10]*=d;g[12]=a;g[13]=b;g[14]=c;g[15]=1;return g},getWorldTransform:function(){return this._getWorldTransform(mat4.create())},_getWorldTransform:function(a){mat4.mul(a,this.getModelTransform(),a);this._parent&&this._parent._getWorldTransform(a);return a},render:function(a){this.isVisible()&&TentaGL.SceneNode.filter(this)&&(TentaGL.ViewTrans.push(a),TentaGL.ViewTrans.mul(a,this.getModelTransform()),this.draw(a),TentaGL.ViewTrans.pop(a))},draw:function(a){}};
...
TentaGL.Sprite.prototype={constructor:TentaGL.Sprite,isaSprite:!0,getAnchorXYZ:function(){return this._anchorXYZ},setAnchorXYZ:function(a){this._anchorXYZ=vec4.fromValues(a[0],a[1],a[2],1)},getAnchorX:function(){return this._anchorXYZ[0]},getAnchorY:function(){return this._anchorXYZ[1]},getAnchorZ:function(){return this._anchorXYZ[2]},firePickEvent:function(a){a=new TentaGL.PickEvent(this,a);for(var b in this._pickEventListeners)this._pickEventListeners[b].handlePickEvent(a)},addPickEventListener:function(a){this._pickEventListeners.push(a)},
removePickEventListener:function(a){a=this._pickEventListeners.indexOf(node);if(-1==a)return!1;this._pickEventListeners.splice(a,1);return!0},getTRSTransform:function(){var a=this.getX(),b=this.getY(),c=this.getZ(),d=0-this.getAnchorX(),e=0-this.getAnchorY(),f=0-this.getAnchorZ(),g=this.getScaleUni(),k=this.getScaleX()*g,l=this.getScaleY()*g,g=this.getScaleZ()*g,h=this._transform;mat4.fromQuat(h,this.getQuat());h[0]*=k;h[1]*=k;h[2]*=k;h[4]*=l;h[5]*=l;h[6]*=l;h[8]*=g;h[9]*=g;h[10]*=g;h[12]=d*h[0]+
e*h[4]+f*h[8]+a;h[13]=d*h[1]+e*h[5]+f*h[9]+b;h[14]=d*h[2]+e*h[6]+f*h[10]+c;h[15]=1;return h},draw:function(a){}};Util.Inheritance.inherit(TentaGL.Sprite,TentaGL.SceneNode);TentaGL.BillboardSprite=function(a){TentaGL.Sprite.call(this,a)};
...
0,h=g.getWidth()-1,m=!1;l<h&&!m;){for(k=0;k<g.getHeight();k++)if(0!=g.getPixelAt(l,k)[3]){m=!0;break}m||l++}for(m=!1;h>l&&!m;){for(k=0;k<g.getHeight();k++)if(0!=g.getPixelAt(h,k)[3]){m=!0;break}m||h--}k=h-l+1;g=g.crop(l,0,k,d);this._charWidths[f]=k}this._charPixData[f]=g;g.toCanvas()}},isMonospaced:function(){return this._monospaced},getLineHeight:function(){return this._charH},getCharPixelData:function(a){return this._charPixData[a]},getCharWidth:function(a){return this._charWidths[a]},getHorizontalPadding:function(){return this._hPad},
getVerticalPadding:function(){return this._vPad},_createTextures:function(a){for(var b in this._charPixData){var c=this._texIDPrefix+b,d=TentaGL.Texture.fromPixelData(a,this._charPixData[b]);TentaGL.MaterialLib.add(a,c,d)}},getStringDimensions:function(a,b){if(!this._charWidths)return[0,0];var c=0,d=0,e=this.getLineHeight(),f=this.getVerticalPadding(),g=this.getHorizontalPadding(),k=1;b&&(k=b/e);for(var l=a.split("\n"),h=0;h<l.length;h++){for(var m=l[h],n=0,q=0;q<m.length;q++)var r=m.charAt(q),r=
this.getCharWidth(r),n=n+(r+g);n>c&&(c=n);d+=e+f}return[c*k,d*k]},renderString:function(a,b,c,d,e){if(this._charPixData){TentaGL.MaterialLib.has(a,this._texIDPrefix+" ")||this._createTextures(a);var f=this.getLineHeight(),g=this.getVerticalPadding(),k=this.getHorizontalPadding();TentaGL.ViewTrans.push(a);TentaGL.ViewTrans.translate(a,c);e&&(c=e/f,TentaGL.ViewTrans.scale(a,[c,c]));b=b.split("\n");for(c=0;c<b.length;c++){e=b[c];TentaGL.ViewTrans.push(a);for(var l=0;l<e.length;l++){var h=e.charAt(l),
m=this._texIDPrefix+h,h=this.getCharWidth(h);TentaGL.MaterialLib.use(a,m);TentaGL.ViewTrans.push(a);TentaGL.ViewTrans.scale(a,[h,f]);TentaGL.ViewTrans.updateMVPUniforms(a);d?TentaGL.ModelLib.render(a,"unitSprite"):TentaGL.ModelLib.render(a,"unitPlane");TentaGL.ViewTrans.pop(a);TentaGL.ViewTrans.translate(a,[h+k,0])}TentaGL.ViewTrans.pop(a);d?TentaGL.ViewTrans.translate(a,[0,f+g]):TentaGL.ViewTrans.translate(a,[0,-(f+g)])}TentaGL.ViewTrans.pop(a)}}};
...
TentaGL.BlitteredFont.fromFont=function(a,b,c,d,e){var f="monospace"==a.getType(),g=a.getStringDimensions("W"),k=g[0],g=g[1];c=new TentaGL.BlitteredFont(void 0,f,k,g,c,d);d=1.5<b.getRed()+b.getGreen()+b.getBlue()?new TentaGL.Color.RGBA(0,0,0,1):new TentaGL.Color.RGBA(1,1,1,1);for(var l=TentaGL.Canvas2D.create(16*(k+2),6*(g+2),d),h=32;128>h;h++){var m=1+(h-32)%16*(k+2),n=1+Math.floor((h-32)/16)*(g+2),q=String.fromCharCode(h),r=TentaGL.Canvas2D.create(k,g,d);TentaGL.Canvas2D.drawString(r,q,a,b,0,0);
TentaGL.Canvas2D.drawImage(l,r,m,n)}a=TentaGL.PixelData.fromCanvas(l);a=a.filter(new TentaGL.RGBAFilter.OneColor(b,.5));e&&(a=e(a));c._createChars(a,f,k,g);return c};TentaGL.Font=function(a,b,c){this._name=a;this._type=b;this._size=c;this._italic=this._bold=!1;this._vPad=1};
...
TentaGL.ShaderVar=function(a,b){this._name=a.name;0<=this._name.indexOf("[")&&(this._name=this._name.substring(0,this._name.indexOf("[")));this._size=a.size;this._type=a.type;this._sizeBytes=a.size*TentaGL.glSizeBytes(a.type);this._sizeUnits=TentaGL.glSizeUnits(a.type);this._isArray=1<this._size;this._glProg=b};
...
TentaGL.Uniform.setters[GL_INT_VEC2]=function(a,b){a.uniform2iv(this._location,b)};TentaGL.Uniform.setters[GL_INT_VEC3]=function(a,b){a.uniform3iv(this._location,b)};TentaGL.Uniform.setters[GL_INT_VEC4]=function(a,b){a.uniform4iv(this._location,b)};TentaGL.Uniform.setters[GL_FLOAT_MAT2]=function(a,b){a.uniformMatrix2fv(this._location,!1,b)};TentaGL.Uniform.setters[GL_FLOAT_MAT3]=function(a,b){a.uniformMatrix3fv(this._location,!1,b)};
TentaGL.Uniform.setters[GL_FLOAT_MAT4]=function(a,b){a.uniformMatrix4fv(this._location,!1,b)};TentaGL.Uniform.setters[GL_SAMPLER_2D]=function(a,b){a.uniform1iv(this._location,b)};
...
TentaGL.ShaderLib={clean:function(a){for(var b in a._shaderLib)a._shaderLib[b].clean(a);a._shaderLib={}},reset:function(a){this.clean(a);TentaGL.PickShader.load(a,TentaGL.Picker.SHADER_ID);a._shaderLibLocked=!1},getDefaultShaderPath:function(a){a._shaderDir||(a._shaderDir="./shaders/");return a._shaderDir},setDefaultShaderPath:function(a,b){a._shaderDir=b},add:function(a,b,c){return a._shaderLib[b]=c},remove:function(a,b){a._shaderLib[b].clean(a);delete a._shaderLib[b]},use:function(a,b){if(!a._shaderLibLocked&&
a._shaderLibCurrentName!==b){var c=a._shaderLib[b];c.useMe(a);a._shaderLibCurrentProgram=c;a._shaderLibCurrentName=b;TentaGL.MaterialLib.useNone(a);return c}},current:function(a){return a._shaderLibCurrentProgram},currentName:function(a){return a._shaderLibCurrentName},lock:function(a){a._shaderLibLocked=!0},unlock:function(a){a._shaderLibLocked=!1}};
TentaGL.ShaderProgram=function(a,b,c){b=this._compileShader(a,GL_VERTEX_SHADER,b);c=this._compileShader(a,GL_FRAGMENT_SHADER,c);this._glProg=this._linkProgram(a,b,c);this._uniforms=this._initUniforms(a);this._attributes=this._initAttributes(a);this._showErrors=!0};
...
a.attachShader(d,b);a.attachShader(d,c);a.linkProgram(d);if(!a.getProgramParameter(d,GL_LINK_STATUS))throw Error("Failed to link shader program.");return d},getLocation:function(){return this._glProg},clean:function(a){a.deleteProgram(this._glProg)},useMe:function(a){var b=TentaGL.ShaderLib.current(a);void 0!==b&&b._disableAllAttrs(a);a.useProgram(this._glProg);this._enableAllAttrs(a)},getAttribLocation:function(a,b){return a.getAttribLocation(this._glProg,b)},getUniformLocation:function(a,b){return a.getUniformLocation(this._glProg,
b)},_initUniforms:function(a){var b={},c=a.getProgramParameter(this._glProg,GL_ACTIVE_UNIFORMS);console.log("# Active Uniforms: "+c);for(var d=0;d<c;d++){var e=a.getActiveUniform(this._glProg,d),f=e.name;0<=f.indexOf("[")&&(f=f.substring(0,f.indexOf("[")));var g=a.getUniformLocation(this._glProg,f),g=new TentaGL.Uniform(e,this._glProg,g);console.log("Uniform "+d+": "+f+", type "+TentaGL.glTypeName(e.type)+", size "+e.size+", byteSize "+g.getSizeBytes()+", unitSize "+g.getSizeUnits());b[f]=g}return b},
_initAttributes:function(a){var b={},c=a.getProgramParameter(this._glProg,GL_ACTIVE_ATTRIBUTES);console.log("# Active Attributes: "+c);for(var d=0;d<c;d++){var e=a.getActiveAttrib(this._glProg,d),f=e.name,g=a.getAttribLocation(this._glProg,f),g=new TentaGL.Attribute(e,this._glProg,g);console.log("Attribute "+d+": "+e.name+", type "+TentaGL.glTypeName(e.type)+", size "+e.size+", byteSize "+g.getSizeBytes()+", unitSize "+g.getSizeUnits());b[f]=g}return b},hasUniform:function(a){return this._uniforms[a]?
!0:!1},getUniform:function(a){!this._uniforms[a]&&this._showErrors&&console.log("Uniform variable "+a+" doesn't exist.");return this._uniforms[a]},getAttribute:function(a){return this._attributes[a]},getUniforms:function(){var a=[],b;for(b in this._uniforms)a.push(this._uniforms[b]);return a},getAttributes:function(){var a=[],b;for(b in this._attributes)a.push(this._attributes[b]);return a},getUniNames:function(){var a=[],b;for(b in this._uniforms)a.push(b);return a},getAttrNames:function(){var a=
[],b;for(b in this._attributes)a.push(b);return a},getUniValue:function(a,b){return this._uniforms[b].get(a)},setUniValue:function(a,b,c){(b=this.getUniform(b))&&b.set(a,c)},bindAttr:function(a,b,c,d){this._attributes[b].set(a,c,d)},setAttrGetter:function(a,b){this._attributes[a].setGetter(b)},getAttrStride:function(){if(void 0===this._totalStride){this._totalStride=0;for(var a in this._attributes)this._totalStride+=this._attributes[a].getSizeBytes()}return this._totalStride},_enableAllAttrs:function(a){for(var b in this._attributes)a.enableVertexAttribArray(this._attributes[b].getLocation())},
_disableAllAttrs:function(a){for(var b in this._attributes)a.disableVertexAttribArray(this._attributes[b].getLocation())}};TentaGL.ShaderProgram.srcFromURL=function(a,b,c){a=new XMLHttpRequest;a.open("get",b,!1);a.send();b=a.responseText;a.open("get",c,!1);a.send();return[b,a.responseText]};
...
this._normalUni=this.getUniform("normalTrans");this._startPtUni=this.getUniform("p");this._gradVectorUni=this.getUniform("u");this._colorsUni=this.getUniform("colors");this._breakPtsUni=this.getUniform("breakPts");this._breakPtCountUni=this.getUniform("breakPtCount")};
...
this.getUniform("tex")};TentaGL.NormalShader.prototype={constructor:TentaGL.NormalShader,isaSimpleShader:!0,setMVPTrans:function(a,b){this._mvpUni.set(a,b)},setNormalTrans:function(a,b){this._normalUni.set(a,b)},setTex:function(a,b){this._texUni.set(a,[b])}};TentaGL.NormalShader.load=function(a,b){var c=new TentaGL.NormalShader(a);TentaGL.ShaderLib.add(a,b,c);return c};Util.Inheritance.inherit(TentaGL.NormalShader,TentaGL.ShaderProgram);
...
this._texUni=this.getUniform("tex")};TentaGL.SimpleShader.prototype={constructor:TentaGL.SimpleShader,isaSimpleShader:!0,setMVPTrans:function(a,b){this._mvpUni.set(a,b)},setNormalTrans:function(a,b){this._normalUni.set(a,b)},setTex:function(a,b){this._texUni.set(a,[b])}};TentaGL.SimpleShader.load=function(a,b){var c=new TentaGL.SimpleShader(a);TentaGL.ShaderLib.add(a,b,c);return c};Util.Inheritance.inherit(TentaGL.SimpleShader,TentaGL.ShaderProgram);

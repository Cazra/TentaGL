...
TentaGL.ViewTrans.getCamera(a).getViewVolumeCorners(a.canvas.width,a.canvas.height),d=[];d.push(new TentaGL.Math.Line3D(c[0],c[1]));d.push(new TentaGL.Math.Line3D(c[1],c[2]));d.push(new TentaGL.Math.Line3D(c[2],c[3]));d.push(new TentaGL.Math.Line3D(c[3],c[0]));d.push(new TentaGL.Math.Line3D(c[4],c[5]));d.push(new TentaGL.Math.Line3D(c[5],c[6]));d.push(new TentaGL.Math.Line3D(c[6],c[7]));d.push(new TentaGL.Math.Line3D(c[7],c[4]));d.push(new TentaGL.Math.Line3D(c[0],c[4]));d.push(new TentaGL.Math.Line3D(c[1],
c[5]));d.push(new TentaGL.Math.Line3D(c[2],c[6]));d.push(new TentaGL.Math.Line3D(c[3],c[7]));for(var c=[],e=0;e<d.length;e++){var f=this.segmentIntersection(d[e]);void 0===f||f.isaLine3D||c.push(f)}if(2<c.length){d=new TentaGL.Model(GL_TRIANGLES,GL_NONE);f=this.getParallelVectors();for(e=0;e<c.length;e++){var g=new TentaGL.Vertex(c[e]);g.normal(this._normal);g.st([0,0]);d.addVertex(g)}var g=[],h=mat4.create();mat4.translate(h,h,vec3.negate([],this._pt));e=TentaGL.Math.getQuatFromTo(this._normal,[0,
0,1]);mat4.mul(h,mat4.fromQuat([],e),h);for(e=0;e<c.length;e++)g.push(vec3.transformMat4([],c[e],h)),g[e][2]=0,g[e][3]=e;for(var l=[0,0,0],e=0;e<g.length;e++)vec3.add(l,l,vec3.scale([],g[e],1/g.length));g.sort(function(a,b){var c=vec3.sub([],a,l),d=vec3.cross([],[1,0,0],c)[2],c=TentaGL.Math.vectorAngle([1,0,0],c);0>d&&(c=TentaGL.TAU-c);var e=vec3.sub([],b,l),d=vec3.cross([],[1,0,0],e)[2],e=TentaGL.Math.vectorAngle([1,0,0],e);0>d&&(e=TentaGL.TAU-e);return e-c});for(e=2;e<g.length;e++)d.addFace(g[0][3],
g[e-1][3],g[e][3]);for(e=0;e<c.length;e++)g=d.getVertex(e),g.tangental([f[0]]);d.render(a)}TentaGL.ViewTrans.pop(a)}};Util.Inheritance.inherit(TentaGL.Math.Plane,TentaGL.Math.Shape3D);TentaGL.Math.Quad=function(a,b,c){this._pts=[];this._pts[0]=a;this._u=b;this._v=c;this._update()};
...
TentaGL.Model._nextID=0;TentaGL.Model.createID=function(){var a="anonID"+TentaGL.Model._nextID;TentaGL.Model._nextID++;return a};
TentaGL.Model.Cone=function(a,b,c){var d=new TentaGL.Model;c||(c=TentaGL.TAU/20);for(var e=Math.floor(TentaGL.TAU/c)+1,f=0;f<e;f++){var g=TentaGL.Math.toCartesian([a,f*c,0]),h=g[0],l=g[2],g=(h/a+1)/2,k=(l/a+1)/2,h=new TentaGL.Vertex(h,0,l);h.setNormal(0,-1,0);h.setTexST(g,k);d.addVertex(h)}for(f=0;f<e;f++){var g=f*c,m=TentaGL.Math.toCartesian([a,g,0]),k=m[0],l=m[2],h=(f+.5)*c,p=TentaGL.Math.toCartesian([a,h,0]),n=p[0],r=p[2],m=vec3.sub(vec3.create(),[0,b,0],m),q=vec3.fromValues(k,0,l),q=vec3.cross(vec3.create(),
q,m),m=vec3.cross(vec3.create(),m,q);vec3.normalize(m,m);p=vec3.sub(vec3.create(),[0,b,0],p);n=vec3.fromValues(n,0,r);n=vec3.cross(vec3.create(),n,p);n=vec3.cross(vec3.create(),p,n);vec3.normalize(n,n);k=new TentaGL.Vertex(k,0,l);k.setTexST(g/TentaGL.TAU,0);k.setNormal(m[0],m[1],m[2]);g=new TentaGL.Vertex(0,b,0);g.setTexST(h/TentaGL.TAU,1);g.setNormal(n[0],n[1],n[2]);d.addVertex(k);d.addVertex(g)}m=TentaGL.Math.toCartesian([a,0,0]);k=m[0];l=m[2];m=vec3.sub(vec3.create(),[0,b,0],m);q=vec3.fromValues(k,
0,l);q=vec3.cross(vec3.create(),q,m);m=vec3.cross(vec3.create(),m,q);vec3.normalize(m,m);k=new TentaGL.Vertex(k,0,l);k.setTexST(TentaGL.TAU,0);k.setNormal(m[0],m[1],m[2]);d.addVertex(k);for(a=2;a<e;a++)b=0,c=a-1,f=a,d.addFace(b,f,c);for(a=0;a<2*e;a+=2)b=e+(a+0),c=e+(a+2),f=e+(a+1),d.addFace(b,c,f);return d};
TentaGL.Model.Cube=function(a,b,c){var d=new TentaGL.Model,e=0,f=function(a,b,c){var f=vec3.cross(vec3.create(),b,c),m=new TentaGL.Vertex(a[0],a[1],a[2]);m.setNormal(f[0],f[1],f[2]);m.setTexST(0,0);d.addVertex(m);m=new TentaGL.Vertex(a[0]+b[0],a[1]+b[1],a[2]+b[2]);m.setNormal(f[0],f[1],f[2]);m.setTexST(1,0);d.addVertex(m);b=new TentaGL.Vertex(a[0]+b[0]+c[0],a[1]+b[1]+c[1],a[2]+b[2]+c[2]);b.setNormal(f[0],f[1],f[2]);b.setTexST(1,1);d.addVertex(b);a=new TentaGL.Vertex(a[0]+c[0],a[1]+c[1],a[2]+c[2]);
...
TentaGL.Model.Cylinder=function(a,b,c){var d=new TentaGL.Model(!1);c||(c=TentaGL.TAU/20);for(var e=Math.floor(TentaGL.TAU/c)+1,f=0;f<e;f++){var g=f*c,h=TentaGL.Math.toCartesian([a,g,0]),l=h[0],h=h[2],g=(l/a+1)/2,k=(h/a+1)/2,m=new TentaGL.Vertex(l,0,h);m.setNormal(0,-1,0);m.setTexST(g,k);d.addVertex(m)}for(f=0;f<e;f++)g=f*c,h=TentaGL.Math.toCartesian([a,g,0]),l=h[0],h=h[2],g/=TentaGL.TAU,m=new TentaGL.Vertex(l,0,h),m.setNormal(l,0,h),m.setTexST(g,0),d.addVertex(m);for(f=0;f<e;f++)g=f*c,h=TentaGL.Math.toCartesian([a,
g,0]),l=h[0],h=h[2],g/=TentaGL.TAU,m=new TentaGL.Vertex(l,b,h),m.setNormal(l,0,h),m.setTexST(g,1),d.addVertex(m);for(f=0;f<e;f++)g=f*c,h=TentaGL.Math.toCartesian([a,g,0]),l=h[0],h=h[2],g=(l/a+1)/2,k=(h/a+1)/2,m=new TentaGL.Vertex(l,b,h),m.setNormal(0,1,0),m.setTexST(g,k),d.addVertex(m);for(a=2;a<e;a++)b=0,c=a-1,f=a,d.addFace(b,f,c);for(a=0;a<e-1;a++)b=1*e+(a+0),c=1*e+(a+1),f=2*e+(a+1),d.addFaceQuad(b,c,f,2*e+(a+0));for(a=2;a<e;a++)b=3*e,c=3*e+a-1,f=3*e+a,d.addFace(b,c,f);return d};
...
k.replace("  "," ");if(""!=k&&"#"!=k.charAt(0)){k=k.split(" ");k[0]!=g&&(f=0);if("g"==k[0])c[d]=e,e=new TentaGL.Model,d=k[1];else if("v"==k[0]){var m=parseFloat(k[1]),p=parseFloat(k[2]),n=parseFloat(k[3]);e.addVertex(new TentaGL.Vertex(m,p,n))}else if("vn"==k[0])g=e.getVertex(f),m=parseFloat(k[1]),p=parseFloat(k[2]),n=parseFloat(k[3]),g.setNormal(m,p,n);else if("vt"==k[0])g=e.getVertex(f),m=parseFloat(k[1]),p=parseFloat(k[2]),g.setTexST(m,p);else if("f"==k[0])g=parseInt(k[1])-1,m=parseInt(k[2])-1,
...
TentaGL.Vertex.prototype={constructor:TentaGL.Vertex,getXYZ:function(){return vec4.clone(this._xyz)},getX:function(){return this._xyz[0]},getY:function(){return this._xyz[1]},getZ:function(){return this._xyz[2]},setX:function(a){this._xyz[0]=a},setY:function(a){this._xyz[1]=a},setZ:function(a){this._xyz[2]=a},xyz:function(a){void 0!==a&&(this._xyz=vec4.fromValues(a[0],a[1],a[2],1));return vec4.clone(this._xyz)},getTexST:function(){return vec2.clone(this._texST)},hasTexST:function(){return void 0!=
this._texST},setTexST:function(a,b){this._texST=vec2.fromValues(a,b)},getTexS:function(){return this._texST[0]},getTexT:function(){return this._texST[1]},st:function(a){void 0!==a&&(this._texST=vec2.fromValues(a[0],a[1]));return vec2.clone(this._texST)},getNormal:function(){return vec3.clone(this._normal)},hasNormal:function(){return void 0!=this._normal},setNormal:function(a,b,c){this._normal=vec3.fromValues(a,b,c);this._normal=vec3.normalize(this._normal,this._normal)},normal:function(a){void 0!==
a&&(this._normal=vec3.fromValues(a[0],a[1],a[2]));return vec3.clone(this._normal)},getTangental:function(){return vec3.clone(this._tangental)},hasTangental:function(){return void 0!==this._tangental},computeTangental:function(a,b){var c=vec3.sub(vec3.create(),a.xyz(),this.xyz()),d=vec3.sub(vec3.create(),b.xyz(),this.xyz()),e=a.getTexS()-this.getTexS(),f=b.getTexS()-this.getTexS(),g=a.getTexT()-this.getTexT(),h=b.getTexT()-this.getTexT(),e=h*e-g*f;if(0==e)return vec3.fromValues(1,0,0);f=vec3.create();
...
TentaGL.LinearGradientShader=function(a){var b=TentaGL.ShaderLib.getDefaultShaderPath(a),c=this;TentaGL.ShaderLoader.load(b+"simple.vert",b+"gradientLinear.frag",function(b,e){console.log("\nCreating linear gradient shader");TentaGL.ShaderProgram.call(c,a,b,e);c.setAttrGetter("vertexPos",TentaGL.Vertex.prototype.getXYZ);c.setAttrGetter("vertexNormal",TentaGL.Vertex.prototype.getNormal);c.setAttrGetter("vertexTexCoords",TentaGL.Vertex.prototype.getTexST);c._mvpUni=c.getUniform("mvpTrans");c._normalUni=
c.getUniform("normalTrans");c._startPtUni=c.getUniform("p");c._gradVectorUni=c.getUniform("u");c._colorsUni=c.getUniform("colors[0]");c._breakPtsUni=c.getUniform("breakPts[0]");c._breakPtCountUni=c.getUniform("breakPtCount")})};TentaGL.LinearGradientShader.prototype={constructor:TentaGL.LinearGradientShader,isaLinearGradientShader:!0};TentaGL.LinearGradientShader.load=function(a,b){var c=new TentaGL.LinearGradientShader(a);TentaGL.ShaderLib.add(a,b,c);return c};
...
c._normalUni=c.getUniform("normalTrans");c._colorUni=c.getUniform("solidColor");c._texUni=c.getUniform("tex");c._useTexUni=c.getUniform("useTex")})};TentaGL.CircleShader.prototype={constructor:TentaGL.CircleShader,isaCircleShader:!0};TentaGL.CircleShader.load=function(a,b){var c=new TentaGL.CircleShader(a);TentaGL.ShaderLib.add(a,b,c);a._circleShaderProgramName=b;return c};Util.Inheritance.inherit(TentaGL.CircleShader,TentaGL.SimpleShader);TentaGL.LightsShader=function(){};
TentaGL.LightsShader.LIGHT_AMB=1;TentaGL.LightsShader.LIGHT_PT=2;TentaGL.LightsShader.LIGHT_DIR=3;TentaGL.LightsShader.LIGHT_SPOT=4;
...
".cutOffAngleCos");g.spotExp=c.getUniform(h+".spotExp")}c._numLightsUni=c.getUniform("numLights")})};TentaGL.PhongShader.MAX_LIGHTS=16;
...
c._materialUni.emis=c.getUniform("m.emis");c._materialUni.shininess=c.getUniform("m.shininess");c._lightsUni=[];var f=c._lightsUni[0]={};f.type=c.getUniform("light.type");f.pos=c.getUniform("light.pos");f.dir=c.getUniform("light.dir");f.diff=c.getUniform("light.diff");f.spec=c.getUniform("light.spec");f.amb=c.getUniform("light.amb");f.attenA=c.getUniform("light.attenA");f.attenB=c.getUniform("light.attenB");f.attenC=c.getUniform("light.attenC");f.cutOffAngleCos=c.getUniform("light.cutOffAngleCos");
f.spotExp=c.getUniform("light.spotExp")})};TentaGL.PhongShaderMac.MAX_LIGHTS=1;TentaGL.PhongShaderMac.LIGHT_AMB=1;TentaGL.PhongShaderMac.LIGHT_PT=2;TentaGL.PhongShaderMac.LIGHT_DIR=3;TentaGL.PhongShaderMac.LIGHT_SPOT=4;TentaGL.PhongShaderMac.prototype={constructor:TentaGL.PhongShaderMac,isaPhongShaderMac:!0,getMaxLights:function(){return TentaGL.PhongShaderMac.MAX_LIGHTS}};TentaGL.PhongShaderMac.load=function(a,b){var c=new TentaGL.PhongShaderMac(a);TentaGL.ShaderLib.add(a,b,c);return c};
Util.Inheritance.inherit(TentaGL.PhongShaderMac,TentaGL.PhongShader);
TentaGL.PlaneSimpleShader=function(a){var b=TentaGL.ShaderLib.getDefaultShaderPath(a),c=this;TentaGL.ShaderLoader.load(b+"planeSimple.vert",b+"planeSimple.frag",function(b,e){console.log("\nCreating PlaneSimpleShader");TentaGL.ShaderProgram.call(c,a,b,e);c.setAttrGetter("vertexPos",TentaGL.Vertex.prototype.getXYZ);c.setAttrGetter("vertexNormal",TentaGL.Vertex.prototype.getNormal);c.setAttrGetter("vertexTexCoords",TentaGL.Vertex.prototype.getTexST);c._opacityUni=c.getUniform("opacity");c._mvpUni=c.getUniform("mvpTrans");

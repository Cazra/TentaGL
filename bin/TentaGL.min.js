...
c[5]));d.push(new TentaGL.Math.Line3D(c[2],c[6]));d.push(new TentaGL.Math.Line3D(c[3],c[7]));for(var c=[],e=0;e<d.length;e++){var f=this.segmentIntersection(d[e]);void 0===f||f.isaLine3D||c.push(f)}if(0<c.length){d=new TentaGL.Model(GL_TRIANGLES,GL_NONE);f=this.getParallelVectors();for(e=0;e<c.length;e++){var g=new TentaGL.Vertex(c[e]);g.normal(this._normal);g.st(0,0);d.addVertex(g)}for(e=0;e<c.length-2;e++)for(g=e+1;g<c.length-1;g++)for(var h=g+1;h<c.length;h++)d.addFace(e,g,h);for(e=0;e<c.length;e++)g=
d.getVertex(e),g.tangental([f[0]]);c=new TentaGL.VBOData(a,d);TentaGL.ViewTrans.updateMVPUniforms(a);TentaGL.VBORenderer.render(a,c);c.clean(a)}TentaGL.ViewTrans.pop(a)}};Util.Inheritance.inherit(TentaGL.Math.Plane,TentaGL.Math.Shape3D);TentaGL.Math.Quad=function(a,b,c){this._pts=[];this._pts[0]=a;this._u=b;this._v=c;this._update()};

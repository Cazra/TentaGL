...
a);a=vec3.dot(this._nHat,a);return 0<a?1:0>a?-1:0},ptIsAbove:function(a){return 1==ptRelative(a)},ptIsBelow:function(a){return-1==ptRelative(a)},lineIntersection:function(a){var b=this.getPoint(),c=a.getPt1(),d=this.getNormal(),e=a.getVec3(),b=vec3.dot(d,vec3.sub(vec3.create(),b,c)),d=vec3.dot(d,e);if(0==d){if(0==b)return a}else return a=b/d,vec4.fromValues(c[0]+a*e[0],c[1]+a*e[1],c[2]+a*e[2],1)},getBoundingBox:function(){},render:function(a,b){TentaGL.ViewTrans.push(a);b&&TentaGL.MaterialLib.use(a,
b);TentaGL.ViewTrans.pop(a)}};Util.Inheritance.inherit(TentaGL.Math.Plane,TentaGL.Math.Shape3D);TentaGL.Math.Quad=function(a,b,c){this._pts=[];this._pts[0]=a;this._u=b;this._v=c;this._update()};
...
pt:function(a){void 0!==xy&&(this._pts[0]=a,this._update());return this._pts[0]},u:function(a){void 0!==a&&(this._u=a,this._update());return this._u},v:function(a){void 0!==a&&(this._v=a,this._update());return this._v},containsPt:function(a,b){b||(b=0);var c=mat3.create(),d=this._pts[0];c[0]=this._u[0];c[1]=this._u[1];c[2]=this._u[2];c[3]=this._v[0];c[4]=this._v[1];c[5]=this._v[2];c[6]=d[0];c[7]=d[1];c[8]=d[2];d=mat3.invert(mat3.create(),c);c=vec3.create();vec3.transformMat3(c,a,d);var d=c[0],e=c[1];
return 0<=d&&1>=d&&0<=e&&1>=e&&Math.abs(c[2]-1)<=b},getBoundingBox:function(){for(var a=this._pts[0][0],b=this._pts[0][0],c=this._pts[0][1],d=this._pts[0][1],e=this._pts[0][2],f=this._pts[0][2],g=1;3>=g;g++){var h=this._pts[g];h[0]<a&&(a=h[0]);h[0]>b&&(b=h[0]);h[1]<d&&(d=h[1]);h[1]>c&&(c=h[1]);h[2]<f&&(f=h[2]);h[2]>e&&(e=h[2])}return new TentaGL.Math.Rect3D},render:function(a,b){var c=new TentaGL.VBOData(a,this._model,TentaGL.getDefaultAttrProfileSet());b&&TentaGL.MaterialLib.use(a,b);TentaGL.VBORenderer(a,
c);c.clean()}};Util.Inheritance.inherit(TentaGL.Math.Quad,TentaGL.Math.Shape3D);TentaGL.Math.Rect3D=function(a,b,c,d){this._pt=a.slice(0);this._w=b;this._h=c;this._d=d};
...
(this._h=a);return this._h},depth:function(a){void 0!==a&&(this._d=a);return this._d},containsPt:function(a,b){return!(a[0]<this.x()||a[0]>this.x()+this.width()||a[1]<this.y()||a[1]>this.y()+this.height()||a[2]<this.z()||a[2]>this.z()+this.depth())},getBoundingBox:function(){return this.clone()},render:function(a,b){TentaGL.ViewTrans.push(a);b&&TentaGL.MaterialLib.use(a,b);TentaGL.ViewTrans.translate(a,this._pt);TentaGL.ViewTrans.scale(a,[this._w,this._h,this._d]);TentaGL.ModelLib.render(a,"unitCube");
TentaGL.ViewTrans.pop(a)}};Util.Inheritance.inherit(TentaGL.Math.Rect3D,TentaGL.Math.Shape3D);TentaGL.Math.Sphere=function(a,b){b||(b=[0,0,0]);this._pt=vec3.clone(b);this._radius=a};

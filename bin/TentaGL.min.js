...
var TentaGL={versionMajor:0,versionMinor:19,createGL:function(a,b){b=b||{alpha:!1};try{var c=a.getContext("webgl",b)||a.getContext("experimental-webgl",b)||a.getContext("webkit-3d",b)||a.getContext("moz-webgl",b);TentaGL.Blend.reset(c);TentaGL.ColorBuffer.reset(c);TentaGL.Cull.reset(c);TentaGL.DepthBuffer.reset(c);TentaGL.Scissor.reset(c);TentaGL.Stencil.reset(c);TentaGL.Viewport.reset(c);TentaGL.ViewTrans.reset(c);this._normalTrans=mat3.create();this._mvpTrans=mat4.create();return c}catch(d){throw Error("Error creating WebGL context: "+
...
a);a=vec3.dot(this._nHat,a);return 0<a?1:0>a?-1:0},ptIsAbove:function(a){return 1==ptRelative(a)},ptIsBelow:function(a){return-1==ptRelative(a)},lineIntersection:function(a){var b=this.getPoint(),c=a.getPt1(),d=this.getNormal(),e=a.getVec3(),b=vec3.dot(d,vec3.sub(vec3.create(),b,c)),d=vec3.dot(d,e);if(0==d){if(0==b)return a}else return a=b/d,vec4.fromValues(c[0]+a*e[0],c[1]+a*e[1],c[2]+a*e[2],1)}};TentaGL.Math.Quad=function(a,b,c){this._pts=[];this._pts[0]=a;this._u=b;this._v=c;this._update()};
TentaGL.Math.Quad.prototype={constructor:TentaGL.Math.Quad,isaQuad:!0,_update:function(){this._pts[1]=vec3.add(vec3.create(),this._pts[0],this._u);this._pts[2]=vec3.add(vec3.create(),this._pts[1],this._v);this._pts[3]=vec3.add(vec3.create(),this._pts[0],this._v);this._n=vec3.cross(vec3.create(),u,v);vec3.normalize(this._n,this._n);this._initModel()},_initModel:function(){this._model=new TentaGL.Model(GL_TRIANGLES,GL_NONE);var a=new TentaGL.Vertex(this._pts[0]);a.setNormal(this._n[0],this._n[1],this._n[2]);
a.setTexST(0,0);this._model.addVertex(a);a=new TentaGL.Vertex(this._pts[1]);a.setNormal(this._n[0],this._n[1],this._n[2]);a.setTexST(1,0);this._model.addVertex(a);a=new TentaGL.Vertex(this._pts[2]);a.setNormal(this._n[0],this._n[1],this._n[2]);a.setTexST(1,1);this._model.addVertex(a);a=new TentaGL.Vertex(this._pts[3]);a.setNormal(this._n[0],this._n[1],this._n[2]);a.setTexST(0,1);this._model.addVertex(a);this._model.addFaceQuad(0,1,2,3)},getPt:function(a){a=TentaGL.Math.clamp(a,0,3);return this._pts[a]},
pt:function(a){void 0!==xy&&(this._pts[0]=a,this._update());return this._pts[0]},u:function(a){void 0!==a&&(this._u=a,this._update());return this._u},v:function(a){void 0!==a&&(this._v=a,this._update());return this._v},render:function(a,b){var c=new TentaGL.VBOData(a,this._model,TentaGL.getDefaultAttrProfileSet());b&&TentaGL.MaterialLib.use(a,b);TentaGL.VBORenderer(a,c);c.clean()}};Util.Inheritance.inherit(TentaGL.Math.Quad,TentaGL.Renderable);
TentaGL.Math.Rect2D=function(a,b,c){void 0===c&&(c=b);this._xy=a;this._w=b;this._h=c};
TentaGL.Math.Rect2D.prototype={constructor:TentaGL.Math.Rect2D,isaRect2D:!0,xy:function(a){void 0!==a&&(this._xy=a);return this._xy},x:function(a){void 0!==a&&(this._xy[0]=a);return this._xy[0]},y:function(a){void 0!==a&&(this._xy[1]=a);return this._xy[1]},width:function(a){void 0!==a&&(this._w=a);return this._w},height:function(a){void 0!==a&&(this._h=a);return this._h},xywh:function(a){void 0!==a&&(this._xy=a.slice(0,2),this._w=a[2],this._h=a[3]);return[this._xy[0],this._xy[1],this._w,this._h]},
containsPt:function(a){return a[0]<this._xy[0]||a[1]<this._xy[1]||a[0]>this._xy[0]+this._w||a[1]>this._xy[1]+this._h?!1:!0},intersection:function(a){if(this.intersectsRect(a)){var b=Math.max(this._xy[0],a._xy[0]),c=Math.min(this._xy[0]+this._w,a._xy[0]+a._w),d=Math.max(this._xy[1],a._xy[1]);a=Math.min(this._xy[1]+this._h,a._xy[1]+a._h);return new TentaGL.Math.Rect2D([b,d],c-b,a-d)}},intersectsRect:function(a){return!(this._xy[0]>a._xy[0]+a._w||this._xy[0]+this._w<a._xy[0]||this._xy[1]>a._xy[1]+a._h||
this._xy[1]+this._h<a._xy[1])},intersectsLine:function(a){if(this.containsPt(a.getPt1())||this.containsPt(a.getPt2()))return!0;var b=this._xy[1]+this._h,c=this._xy[1],d=this._xy[0],e=this._xy[0]+this._w;return a.intersects(new TentaGL.Math.Line2D([d,b],[e,b]))||a.intersects(new TentaGL.Math.Line2D([d,c],[e,c]))||a.intersects(new TentaGL.Math.Line2D([d,b],[d,c]))||a.intersects(new TentaGL.Math.Line2D([e,b],[e,c]))},render:function(a,b){TentaGL.ViewTrans.push(a);b&&TentaGL.MaterialLib.use(a,b);TentaGL.ViewTrans.translate(a,
this._xy);TentaGL.ViewTrans.scale(a,[this._w,this._h]);TentaGL.ModelLib.render(a,"unitPlane");TentaGL.ViewTrans.pop(a)}};Util.Inheritance.inherit(TentaGL.Math.Rect2D,TentaGL.Renderable);TentaGL.Math.Sphere=function(a,b){b||(b=[0,0,0]);this._pt=vec3.clone(b);this._radius=a};
...
TentaGL.RGBAFilter.Convolution.prototype={constructor:TentaGL.RGBAFilter.Convolution,getMatrix:function(){return this._matrix},getSideLength:function(){return this._sideLen},filter:function(a,b,c,d,e){for(var f=0,g=0,h=0,m=b[c+3],k=this._sideLen,l=TentaGL.PixelData.indexToXY(c,d),p=l[0],l=l[1],n=Math.floor(k/2),r=0;r<k;r++){var q=l+r-n;q>=e&&(q=e-1);0>q&&(q=0);for(var t=0;t<k;t++){var w=this._matrix[r][t],s=p+t-n;s>=d&&(s=d-1);0>s&&(s=0);var s=TentaGL.PixelData.xyToIndex(s,q,d),x=b[s+3]/255,f=f+b[s]*
w*x,g=g+b[s+1]*w*x,h=h+b[s+2]*w*x}}f=TentaGL.Math.clamp(f,-256,255);0>f&&(f+=256);g=TentaGL.Math.clamp(g,-256,255);0>g&&(g+=256);h=TentaGL.Math.clamp(h,-256,255);0>h&&(h+=256);this.setPixel(a,c,f,g,h,m)}};Util.Inheritance.inherit(TentaGL.RGBAFilter.Convolution,TentaGL.RGBAFilter);TentaGL.RGBAFilter.Convolution.Blur3=function(){return new TentaGL.RGBAFilter.Convolution([[1,1,1],[1,1,1],[1,1,1]])};

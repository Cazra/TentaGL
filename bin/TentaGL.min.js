...
a,this._p2);if(0>=vec3.dot(b,c))return vec3.len(c);if(0<=vec3.dot(b,a))return vec3.len(a);b=vec3.normalize(vec3.create(),b);a=vec3.normalize(vec3.create(),c);b=vec3.cross(vec3.create(),b,a);return vec3.len(c)*vec3.len(b)},render:function(a,b){var c=vec3.clone(this._p1);vec3.clone(this._p2);var d=this.getVec3();TentaGL.ViewTrans.push(a);TentaGL.ViewTrans.translate(a,c);TentaGL.ViewTrans.scale(a,d);TentaGL.ViewTrans.updateMVPUniforms(a);b&&TentaGL.MaterialLib.use(a,b);TentaGL.ModelLib.render(a,"unitLine");
TentaGL.ViewTrans.pop(a)}};TentaGL.Math.Plane=function(a,b){b||(b=[0,0,0]);this._normal=vec3.clone(a);this._nHat=vec3.normalize(vec3.create(),this._normal);this._pt=vec3.clone(b)};

...
b=b.concat(this._controlPts);b.push(this._endPt);return this._interpolate(a,b)},_interpolate:function(a,b){if(1==b.length)return b[0];for(var c=[],d=0;d<b.length-1;d++){var e=b[d+1],e=vec2.add([],vec2.scale([],b[d],1-a),vec2.scale([],e,a));c.push(e)}return this._interpolate(a,c)},containsPt:function(a){},getBounds2D:function(){for(var a=Math.min(this._startPt[0],this._endPt[0]),b=Math.max(this._startPt[0],this._endPt[0]),c=Math.min(this._startPt[1],this._endPt[1]),d=Math.max(this._startPt[1],this._endPt[1]),
e=0;e<this._controlPts;e++)var f=this._controlPts[e],a=Math.min(a,f[0]),b=Math.max(b,f[0]),c=Math.min(c,f[1]),d=Math.max(d,f[1])},render:function(a,b,c){b&&TentaGL.MaterialLib.use(a,b);void 0===c&&(c=10*this.getDegree());2>c&&(c=2);b=[];b.push(this.start());for(var d=c=1/(c-1);1>d;d+=c)b.push(this.interpolate(d));b.push(this.end());for(c=0;c<b.length-1;c++)(new TentaGL.Math.Line2D(b[c],b[c+1])).render(a)}};Util.Inheritance.inherit(TentaGL.Math.BezierCurve2D,TentaGL.Math.Shape2D);
...
TentaGL.Model.Cone=function(a,b,c){var d=new TentaGL.Model;c||(c=TentaGL.TAU/20);for(var e=Math.floor(TentaGL.TAU/c)+1,f=0;f<e;f++){var g=TentaGL.Math.toCartesian([a,f*c,0]),h=g[0],l=g[2],g=(h/a+1)/2,k=(l/a+1)/2,h=new TentaGL.Vertex([h,0,l]);h.normal([0,-1,0]);h.texST([g,k]);d.addVertex(h)}for(f=0;f<e;f++){var g=f*c,m=TentaGL.Math.toCartesian([a,g,0]),k=m[0],l=m[2],h=(f+.5)*c,p=TentaGL.Math.toCartesian([a,h,0]),n=p[0],r=p[2],m=vec3.sub(vec3.create(),[0,b,0],m),q=vec3.fromValues(k,0,l),q=vec3.cross(vec3.create(),
q,m),m=vec3.cross(vec3.create(),m,q);vec3.normalize(m,m);p=vec3.sub(vec3.create(),[0,b,0],p);n=vec3.fromValues(n,0,r);n=vec3.cross(vec3.create(),n,p);n=vec3.cross(vec3.create(),p,n);vec3.normalize(n,n);k=new TentaGL.Vertex([k,0,l]);k.texST([g/TentaGL.TAU,0]);k.normal(m);g=new TentaGL.Vertex([0,b,0]);g.texST([h/TentaGL.TAU,1]);g.normal(n);d.addVertex(k);d.addVertex(g)}m=TentaGL.Math.toCartesian([a,0,0]);k=m[0];l=m[2];m=vec3.sub(vec3.create(),[0,b,0],m);q=vec3.fromValues(k,0,l);q=vec3.cross(vec3.create(),
...
k.replace("  "," ");if(""!=k&&"#"!=k.charAt(0)){k=k.split(" ");k[0]!=g&&(f=0);if("g"==k[0])c[d]=e,e=new TentaGL.Model,d=k[1];else if("v"==k[0]){var m=parseFloat(k[1]),p=parseFloat(k[2]),n=parseFloat(k[3]);e.addVertex(new TentaGL.Vertex([m,p,n]))}else if("vn"==k[0])g=e.getVertex(f),m=parseFloat(k[1]),p=parseFloat(k[2]),n=parseFloat(k[3]),g.normal([m,p,n]);else if("vt"==k[0])g=e.getVertex(f),m=parseFloat(k[1]),p=parseFloat(k[2]),g.texST([m,p]);else if("f"==k[0])g=parseInt(k[1])-1,m=parseInt(k[2])-1,
p=parseInt(k[3])-1,5==k.length?(n=parseInt(k[4])-1,e.addFaceQuad(g,m,p,n)):e.addFace(g,m,p);else if("usemtl"!=k[0])throw Error("Could not read wavefront object. Error at line "+(l+1));f++;g=k[0]}}c[d]=e;b(c)},isLoading:function(){return 0<this._numLoading},getNumLoading:function(){return this._numLoading}};
...
TentaGL.RGBAFilter.OutlineColor.prototype={constructor:TentaGL.RGBAFilter.OutlineColor,getColor:function(){return this._color},filter:function(a,b,c,d,e){var f=b[c],g=b[c+1],h=b[c+2],l=b[c+3];if(255!=l)for(var k=TentaGL.PixelData.indexToXY(c,d),m=k[0],k=k[1],p=-1;1>=p;p++){var n=k+p;if(!(n>=e||0>n))for(var r=-1;1>=r;r++){var q=m+r;if(!(q>=d||0>q)&&(q=TentaGL.PixelData.xyToIndex(q,n,d),255==b[q+3]))return f=this._color.getRedByte(),g=this._color.getGreenByte(),h=this._color.getBlueByte(),this.setPixel(a,
...
TentaGL.BlitteredFont.fromFont=function(a,b,c,d,e){var f="monospace"==a.getType(),g=a.getStringDimensions("W"),h=Math.floor(g[0]),g=Math.floor(g[1]);c=new TentaGL.BlitteredFont(void 0,f,h,g,c,d);d=1.5<b.r()+b.g()+b.b()?TentaGL.Color.BLACK:TentaGL.Color.WHITE;for(var l=TentaGL.Canvas2D.create(16*(h+2),6*(g+2),d),k=32;128>k;k++){var m=1+(k-32)%16*(h+2),p=1+Math.floor((k-32)/16)*(g+2),n=String.fromCharCode(k),r=TentaGL.Canvas2D.create(h,g,d);TentaGL.Canvas2D.drawString(r,n,a,b,0,0);TentaGL.Canvas2D.drawImage(l,
r,m,p)}a=TentaGL.PixelData.fromCanvas(l);a=a.filter(new TentaGL.RGBAFilter.OneColor(b,.5));e&&(a=e(a));c._createChars(a,f,h,g);return c};TentaGL.Font=function(a,b,c){this._name=a;this._type=b;this._size=c;this._italic=this._bold=!1;this._vPad=1};

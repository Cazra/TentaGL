...
0,1]);mat4.mul(h,mat4.fromQuat([],e),h);for(e=0;e<c.length;e++)g.push(vec3.transformMat4([],c[e],h)),g[e][2]=0,g[e][3]=e;for(var m=[0,0,0],e=0;e<g.length;e++)vec3.add(m,m,vec3.scale([],g[e],1/g.length));g.sort(function(a,b){var c=vec3.sub([],a,m),d=vec3.cross([],[1,0,0],c)[2],c=TentaGL.Math.vectorAngle([1,0,0],c);0>d&&(c=TentaGL.TAU-c);var e=vec3.sub([],b,m),d=vec3.cross([],[1,0,0],e)[2],e=TentaGL.Math.vectorAngle([1,0,0],e);0>d&&(e=TentaGL.TAU-e);return e-c});console.log(g);for(e=2;e<g.length;e++)d.addFace(g[0][3],
g[e-1][3],g[e][3]);for(e=0;e<c.length;e++)g=d.getVertex(e),g.tangental([f[0]]);c=new TentaGL.VBOData(a,d);TentaGL.ViewTrans.updateMVPUniforms(a);TentaGL.VBORenderer.render(a,c);c.clean(a)}TentaGL.ViewTrans.pop(a)}};Util.Inheritance.inherit(TentaGL.Math.Plane,TentaGL.Math.Shape3D);TentaGL.Math.Quad=function(a,b,c){this._pts=[];this._pts[0]=a;this._u=b;this._v=c;this._update()};
...
TentaGL.VBOData=function(a,b,c){c||(c=TentaGL.getDefaultAttrProfileSet());var d=[];this._byteOffsets={};for(var e=b.getVertices(),f=0;f<e.length;f++){var g=e[f],h=0,m;for(m in c){var k=c[m],l=k.getValues(g);this._byteOffsets[m]=h;h+=k.sizeBytes();for(k=0;k<l.length;k++)d.push(l[k])}}this._attrBuffer=a.createBuffer();a.bindBuffer(GL_ARRAY_BUFFER,this._attrBuffer);a.bufferData(GL_ARRAY_BUFFER,new Float32Array(d),GL_STATIC_DRAW);d=b.getIndices();this._numIndices=b.numIndices();this._elemBuffer=a.createBuffer();
